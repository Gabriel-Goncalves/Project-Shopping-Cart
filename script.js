...
function cartItemClickListener() {
  addEventListener('click', (event) => {
    if (event.target.className === 'cart__item') {
      event.target.remove();
      updateLocalStorage();
      document.querySelector('.total-price').innerText = `Preço Total = R$ ${sumTotalPrice()}`;
    }
  });
}
...
function addElementToCart(result) {
  ...
  document.querySelector(
    '.total-price',
  ).innerText = `Preço Total = R$ ${sumTotalPrice()}`;
}
...
function updateCart() {
  if (localStorage.list !== undefined) {
    ...
    document.querySelector(
      '.total-price',
    ).innerText = `Preço Total = R$ ${totalPrice}`;
  }
}
...
function buttonClearCart() {
  ...
  btnClear.addEventListener('click', () => {
    ...
    updateLocalStorage();
    document.querySelector('.total-price').innerText = 'Preço Total = R$ 0';
  });
}
